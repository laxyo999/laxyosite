...
<div class="row">
  <div class="col-md-12">
    ...
    <div style="float:right;">
     <a href="{{route('admin.ImgUpload.create')}}" class="btn btn-success">New</a>
     </div>
  </div>
</div>
<div class="uper">
  ...
  <table class="table table-striped table-bordered bg-light">
    ...
    <tbody>
        ...
        <tr>
            ...
            <td>{{$post->catg_id}}</td>
            <td>{{$post->title}}</td>
            <td><img src="/images/{{$post->imagepath}}" alt="img" width="100px"></td>
            <td><a href="/admin/ImgUpload/{{$post->id}}/edit" class=" btn btn-success">Edit</a>
                <a href="#" class="btn btn-danger" onclick="event.preventDefault(); if(confirm('Are you sure?')){
                  ...
                  <form id="delete-form-{{ $post->id }}" action="{{ route('admin.ImgUpload.destroy', $post->id) }}" method="POST" style="display: none;">
                      @csrf
                      @method('delete')
                  </form>
              </td>
